...
def fake_guild(members=None):
	"""A guild whose member cache only knows the ids we were given.

	This is the realistic shape: with Intents.default() anyone already in voice
	when the process started is absent from the cache, and a VoiceState carries no
	member object to fall back on.
	"""
	cache = members or {}
	return types.SimpleNamespace(id=1, get_member=cache.get)


async def test_humans_detected_from_voice_states_when_member_cache_is_empty():
	cog = make_cog()
	channel = types.SimpleNamespace(members=[], voice_states={42: object()}, guild=fake_guild())
...
async def test_voice_states_ignore_the_bot_and_other_bots():
	...
	channel = types.SimpleNamespace(
		members=[],
		voice_states={999: object(), 100: object()},
		guild=fake_guild({100: types.SimpleNamespace(bot=True)}),
	)

	assert not cog._channel_has_humans(channel)


async def test_a_resolvable_human_keeps_the_bot_around():
	cog = make_cog()
	channel = types.SimpleNamespace(
		members=[],
		voice_states={999: object(), 100: object()},
		guild=fake_guild({100: types.SimpleNamespace(bot=False)}),
	)

	assert cog._channel_has_humans(channel)


async def test_rejoin_resumes_a_stranded_queue(make_player, make_track):
	"""A queue left behind by a voice outage must restart when someone rejoins."""
	import asyncio
	...
	channel.voice_states = {42: object()}
	channel.guild = fake_guild()

	track = make_track('t1')
	player.queue.append(track)      # stranded: queued, but no driver owns it

	member = types.SimpleNamespace(id=42, guild=types.SimpleNamespace(id=1), bot=False)
...
async def test_bot_leaving_does_not_tear_down_immediately(make_player, make_track):
	"""py-cord emits this event mid-reconnect; acting on it detaches the player."""
	import asyncio
	...
	member = types.SimpleNamespace(id=999, guild=types.SimpleNamespace(id=1), bot=True)
	before = types.SimpleNamespace(channel=player.voice_client.channel)
